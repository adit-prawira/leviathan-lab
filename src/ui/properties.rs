        ...
        egui::SidePanel::right("properties").min_width(MIN_SIDE_PANEL_WIDTH)
            .show(contexts.ctx_mut().expect("egui context to be available"),|ui| {
                ...
                egui::Grid::new("Mode").spacing([8.0, 8.0]).show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.selectable_value(&mut local_mode, GizmosMode::Translate, "Translate (T)");
                        ui.selectable_value(&mut local_mode, GizmosMode::Scale, "Scale (S)");
                        ui.selectable_value(&mut local_mode, GizmosMode::Rotate, "Rotate (R)");
                    });
                    if local_mode != current_mode {*mode = local_mode;};
                    ui.end_row();
